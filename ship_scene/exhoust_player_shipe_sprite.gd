...
var current_turb: int = 0;

func change_exhaust():
	texture = load(image_path + turbo_status[current_turb] + "/Exhaust1" + images[current_image]);
	print(image_path + turbo_status[current_turb] + "/Exhaust1" + images[current_image]);
	current_image += 1;
	if current_image >= 4:
		current_image = 0;
